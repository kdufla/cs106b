...
private:
	...
	int alSize;
	string *myQueue;
	/* if small value is too low moves it up, where it needs to be */
	void bubbleUp(int n);
	/* substitute elements n1 and n2 in  queue */
	void substitute(int n1, int n2);
	/* if large value is too high moves it down, where it needs to be */
